...
def train(input_variable, lengths, target_variable, mask, max_target_len, encoder, decoder, embedding,
    ...
    testenco_outputs, testenco_hidden = testenco(input_variable, lengths, None)

    decoder_input = torch.LongTensor([[SOS_token for _ in range(batch_size)]])
    decoder_input = decoder_input.to(device)

    decoder_hidden = encoder_hidden[:decoder.n_layers]
    test_hidden = testenco_hidden[:decoder.n_layers]
...
#    loss = loss * score.item()
   ...
   # loss += criterion(state_prob, score[0][0])
    ...
    return loss.item() / max_target_len, np.mean(score)
...
def trainIters(corpus, reverse, n_iteration, learning_rate, batch_size, n_layers, hidden_size,
    ...
    for iteration in tqdm(range(start_iteration, n_iteration + 1)):
        ...
        loss, score = train(input_variable, lengths, target_variable, mask, max_target_len, encoder,
    ...
    saveloss = np.array(saveloss)
    savescore = np.array(savescore)

    plt.plot(savescore)
    plt.savefig('baseentroscore.png')
    plt.clf()
    plt.plot(saveloss)
    plt.savefig('baseentroloss.png')
